...
def extract_boundary_edges_safe(faces):
    ...
    for quad in faces:
            ...
            edges = [
                (min(quad[1], quad[2]), max(quad[1], quad[2])),
                (min(quad[2], quad[3]), max(quad[2], quad[3])),
                (min(quad[3], quad[4]), max(quad[3], quad[4])),
                (min(quad[4], quad[1]), max(quad[4], quad[1])),
            ]
            
            for edge in edges:
                edge_usage[edge] += 1
    boundary_edges = [edge for edge, count in edge_usage.items() if count == 1]
    print(f"find {len(boundary_edges)} boundaries")
        
    return boundary_edges

def add_boundary_facets(faces, v_num):

    boundary_edges = extract_boundary_edges_safe(faces)
    facets_boundary = []
    for edge in boundary_edges:
        v_0 = edge[0]
        v_1 = edge[1]

        v_bottom_0 = v_0 + v_num
        v_bottom_1 = v_1 + v_num
        facets_boundary.append([4, v_1, v_0, v_bottom_0, v_bottom_1])

    return np.array(facets_boundary)
...
def construct_facets_from(mask, invert_dir=False):
    ...
    if not invert_dir:
        ...
        return np.stack((4 * np.ones(np.sum(facet_top_left_mask)),
               ...
               idx[facet_bottom_right_mask],
               idx[facet_bottom_left_mask]), axis=-1).astype(int)

def map_depth_map_to_point_clouds_normal(depth_map, normal_map, mask, K=None, step_size=1):
    # y
    # |  z
    # | /
    # |/
...
def bilateral_normal_integration_bi_mask(normal_map,
                                 normal_mask,
                                 k=2,
                                 depth_map=None,
                                 depth_mask=None,
                                 depth_mask_focus=None,
                                 lambda1=1e-4,
                                 lambda2=1e-4,
                                 K=None,
                                 step_size=1,
    ...
    if depth_mask_focus is not None:
        print('depth focus is not none')
        m_focus = depth_mask_focus[normal_mask].astype(int)
        print("m_focus shape")
        print(m_focus.shape)
        M_focus = spdiags(m_focus, 0, num_normals, num_normals, format="csr")
        print("M_focus shape")
        print(M_focus.shape)
        # z_prior_focus = np.log(depth_map)[normal_mask] if K is not None else depth_map[normal_mask]
    ...
    for i in pbar:
        ...
        if depth_map is not None:
            ...
            z = z + offset
            A_mat += (lambda1 * M + lambda2 * M_focus)
            b_vec += (lambda1 * M + lambda2 * M_focus) @ z_prior
    ...
    # depth_map_out[depth_mask] = depth_map
    depth_map_out[normal_mask] = z

    valid_depths = depth_map[normal_mask]
    d_min = np.min(valid_depths)
    d_max = np.max(valid_depths)
    
    # 获取计算深度图的有效范围
    valid_new_depths = depth_map_out[normal_mask]
    new_d_min = np.min(valid_new_depths)
    new_d_max = np.max(valid_new_depths)
    
    # 计算缩放系数（避免除零）
    if np.abs(new_d_max - new_d_min) > 1e-10:
        scale = (d_max - d_min) / (new_d_max - new_d_min)
        
        # 应用对齐变换：先平移，后缩放（合并为一步）
        # 公式：aligned_d = (new_d - new_d_min) * scale + d_min
        depth_map_out[normal_mask] = (depth_map_out[normal_mask] - new_d_min) * scale + d_min
        
        print(f"Depth alignment applied: scale={scale:.4f}, "
            f"Input range: [{d_min:.4f}, {d_max:.4f}], "
            f"Output range before: [{new_d_min:.4f}, {new_d_max:.4f}], "
            f"after: [{np.min(depth_map_out[normal_mask]):.4f}, {np.max(depth_map_out[normal_mask]):.4f}]")

    if K is not None:  # perspective
        depth_map_out = np.exp(depth_map_out)
    ...
    # vertices, normals = map_depth_map_to_point_clouds_normal(depth_map_out, normal_map, normal_mask, K=None, step_size=step_size)

    # print(depth_map_out.shape)
    ...
    #add bottom
    vertices_bottom = vertices.copy()
    min_z = np.min(vertices[:, 2])
    print("the minimum z value is")
    print(min_z)
    base_offset = 20.0
    base_height = min_z - base_offset
    vertices_bottom[:,2] = base_height
    num_vertices = vertices.shape[0]
    vertices_all = np.concatenate((vertices, vertices_bottom), axis=0)
    facets_bottom = construct_facets_from(normal_mask, invert_dir=True)
    print("facet shape is ")
    print(facets_bottom.shape)
    facets_bottom[:, 1:] += num_vertices
    facets_boundary = add_boundary_facets(facets, num_vertices)
    facets_all = np.concatenate((facets, facets_bottom, facets_boundary), axis=0)
    surface = pv.PolyData(vertices_all, facets_all)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l1', '--lam1', type=float, default=1e-3)
    parser.add_argument('-l2', '--lam2', type=float, default=1e-3)
    ...
    depth_mask = cv2.imread(os.path.join(arg.path, "depth_mask.png"), cv2.IMREAD_GRAYSCALE).astype(bool)
    depth_mask_focus = cv2.imread(os.path.join(arg.path, "depth_mask_focus.png"), cv2.IMREAD_GRAYSCALE).astype(bool)


    # try:
        # depth = cv2.imread(os.path.join(arg.path, "depth.png"), cv2.IMREAD_GRAYSCALE)
        # dmax = 34.0987#for recovering depth
        # dmin = 29.5546
        # scale = 20.
        # depth = depth/255.
        # depth = depth * (dmax-dmin) + dmin
        # depth = depth * scale
    depth_extremes_path = os.path.join(arg.path, "depth_extremes.npy")
    depth_input_path = os.path.join(arg.path, "depth_map.png")
    depth_extremes = np.load(depth_extremes_path, allow_pickle=True).item()
    z_min, z_max,scale = depth_extremes["depth_min"], depth_extremes["depth_max"],depth_extremes["scale"]

    depth_input = cv2.imread(depth_input_path, cv2.IMREAD_GRAYSCALE) 
    normalized_z = depth_input.astype(np.float32) / 255.0
    
    # 还原到原始Z坐标范围
    original_z = normalized_z * (z_max - z_min) + z_min
    depth = (original_z / (2*scale)) * 1024
    # depth_mask = (depth>0)
    # except:
    #     depth = None
    ...
    sparse_depth_mask_focus = create_sparse_depth_mask(depth_mask_focus, pattern='checkerboard')
    cv2.imwrite(os.path.join(arg.path, f"sparse_depth_mask.png"), sparse_depth_mask*255)
    depth_map, surface, wu_map, wv_map, energy_list = bilateral_normal_integration_bi_mask(normal_map=normal_map,
                                                                                    normal_mask=mask,
                                                                                    depth_map= depth,
                                                                                    depth_mask = sparse_depth_mask,#depth_mask,if not use sparse mask
                                                                                    depth_mask_focus = sparse_depth_mask_focus,#depth_mask_focus, if not use sparse mask
                                                                                    k=arg.k,
                                                                                    K=None,
                                                                                    max_iter=arg.iter,
                                                                                    tol=arg.tol,
